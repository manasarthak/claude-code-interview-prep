# Pandas Data Manipulation — Complete Guide
...
Data-engineering coding rounds — at consulting shops, quant firms, product companies, wherever — lean heavily on pandas. The patterns are repeatable: fill-NA-with-group-mean, string extraction/cleaning, nested-JSON ingestion, joins/anti-joins, dedup/data-quality, feature engineering, and paginated-API pulls. This file is a re-learning + reference guide covering the full pandas surface these rounds test, with runnable code, the **SQL→pandas bridge** (your fastest path if your [SQL](./01-sql-deep-dive.md) is solid), and a gotchas list at the end (real problems punish edge cases, not concepts).
...
- **Vectorize, don't loop.** `df['a'] + df['b']` is fast; `df.apply(..., axis=1)` and Python `for` loops are slow and a common reason coding-round solutions time out. The reason is [in the Python internals file](../python/02-python-internals.md): pure-Python loops box every value and pay GIL/dispatch overhead, while vectorized ops run in compiled C/SIMD on contiguous NumPy buffers.
...
**`info()` and `isna().sum()` are your first two moves on any mystery DataFrame in a coding round.**
...
# query() — readable, good for time-pressured code
...
## 3. Missing Data (very common in interviews)
...
### A Very Common Question — Fill NA with the Group Mean
...
**Reach for `np.where` / `np.select` before `apply`.** They're vectorized and far faster — usually 10–100× on a million-row DataFrame. `apply(axis=1)` is the canonical reason a solution times out.

---

## 5. String Operations (common question: "get the character values only")
...
```python
# "keep only the letters" / "keep only the digits" — classic prompts
...
## 9. Merge / Join (and the anti-join pattern)
...
### Common Question — "Customers / Products Without a Sale" (anti-join)
...
When a coding question feels like SQL, write the SQL in your head first, then translate row by row through this table.

---

## 15. The API-Pagination Pattern (memorize the skeleton)

A recurring coding-round pattern: hit a paginated REST endpoint, read `total_pages`, loop, aggregate. The shape is always the same:
...
records = fetch_all('https://example.com/api/countries')
...
Adds: timeout, error raising, defensive `get` with defaults, max-page guard. Useful when the grader tests error handling.

---

## 16. Gotchas Checklist (read the night before)
...
10. **Autograders check exact output** — column order, names, index, dtypes, and rounding all matter. Match the sample output precisely (`round()`, `reset_index(drop=True)`).
...
15. **`pd.to_datetime` without `errors='coerce'`** throws on bad input. Always coerce when input quality is unknown.

---

## 17. Practice Tasks (real-world patterns — try before checking)
...
## 18. Pandas vs PySpark

A growing minority of coding rounds (Databricks-heavy shops, big-data teams) phrase questions in PySpark. The concepts are identical; the API differs. The big mental shifts:
...
For interviews, if you're comfortable in pandas and the dataset is small, `toPandas()` on a filtered Spark DataFrame is a legitimate shortcut. In production, `toPandas()` on a 1B-row DataFrame OOMs the driver — bad pattern. See [Spark Fundamentals](./07-spark-fundamentals.md) for the production discussion.
...
## 19. Performance Notes (when a solution times out)
...
10. **For really big work, consider [Polars](https://pola.rs/) or [DuckDB](https://duckdb.org/)** — same API shape, often 5–30× faster than pandas for medium-sized data. (Most interview graders only ship pandas — know about them, but write pandas for the interview.)
...
*Next: drill the 8 practice tasks above. If you can do all 8 cold in 60 min, the pandas half of any interview is yours.*
