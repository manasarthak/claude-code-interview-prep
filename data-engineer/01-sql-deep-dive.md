...
## ⚠️ OA / HackerRank Engine Warning — Read This Before Drilling

HackerRank, CoderPad, and most online assessments (OAs) run **MySQL, MS SQL Server, Oracle, or DB2** — not Snowflake / BigQuery / Redshift. Many "killer features" of warehouse SQL **do not exist** on these engines:
...
**Rule of thumb for OAs:** before you write a single character, identify the engine (it's shown on the question), and bias toward the *portable* pattern (CTE + window function + standard `CASE`) over any dialect-specific sugar. Full dialect cheat sheet in the [OA Dialect Reference](#oa-dialect-reference) section below.
...
### NULL Handling Functions (the OA constants)

NULL behavior was covered above — these are the **functions** you use to handle it. They appear on nearly every OA SQL question.
...
```sql
-- Replace NULLs in output (every OA: "show 0 instead of NULL for users with no orders")
SELECT u.user_id,
...
The single most common "passes examples, fails hidden tests" bug on OAs. Postgres, SQL Server, Oracle, Spark SQL all use **integer division** when both operands are integers:
...
### String Functions — Constant on OAs, Often Underdrilled

You'll need at least one string function on roughly half of OA SQL questions. The standard set:
...
```sql
-- "Find emails whose domain is gmail.com" — classic OA
SELECT email
FROM users
...
```sql
-- Tagging the source — extremely common OA / DE pattern
...
**⚠️ DO NOT use QUALIFY on HackerRank/OAs.** Their engines (MySQL, MS SQL, Oracle, DB2, sometimes Postgres) don't support it. Always default to the portable pattern:
...
If your OA engine doesn't support `FILTER`, fall back to `SUM(CASE WHEN ... THEN x ELSE 0 END)` — universally supported, slightly noisier.

---

## OA Dialect Reference {#oa-dialect-reference}
...
### The OA "Before You Hit Run" Checklist
...
- `QUALIFY` (Snowflake/BQ/Spark) vs `CTE + WHERE rn = 1`: shorthand vs portable. Default to portable for OAs.
...
- [HackerRank SQL track](https://www.hackerrank.com/domains/sql) — directly the OA format you'll face

### Companion files
- [Pandas Data Manipulation](./16-pandas-data-manipulation.md) — the pandas equivalent for OA-style data wrangling
